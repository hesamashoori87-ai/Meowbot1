...
async def confirm_listing(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    is_admin = user.id == ADMIN_ID
    if callback.message:
        await callback.message.edit_text("✅ آگهی شما با موفقیت ثبت شد و پس از تأیید ادمین منتشر خواهد شد.")
        await callback.message.answer("🏠 منوی اصلی:", reply_markup=main_menu_kb(is_admin=is_admin))
    try:
        ...
        await callback.bot.send_message(ADMIN_ID, notify_text, reply_markup=approve_reject_kb(listing_id))
    except Exception:
        logger.exception("ارسال اعلان آگهی جدید به ادمین با خطا مواجه شد.")
    await callback.answer()
