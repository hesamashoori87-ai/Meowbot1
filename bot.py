...
DEFAULT_CATEGORIES = [
    ...
    "وسایل نقلیه", "املاک", "پوشاک", "میو پوینت", "سایر",
]
...
class Database:
    ...
    async def add_category(self, name: str) -> bool:
        ...
        try:
            ...
            return False  # دسته‌ای با همین اسم از قبل وجود دارد

    async def category_has_listings(self, category_id: int) -> bool:
        assert self.conn is not None
        cur = await self.conn.execute(
            "SELECT 1 FROM listings WHERE category_id = ? LIMIT 1;", (category_id,)
        )
        return (await cur.fetchone()) is not None

    async def delete_category(self, category_id: int) -> bool:
        """فقط در صورتی حذف می‌کند که هیچ آگهی‌ای به این دسته وصل نباشد."""
        assert self.conn is not None
        if await self.category_has_listings(category_id):
            return False
        cur = await self.conn.execute("DELETE FROM categories WHERE id = ?;", (category_id,))
        await self.conn.commit()
    ...
    async def get_latest_listings(self, limit: int = 10, offset: int = 0) -> list[dict[str, Any]]:
        assert self.conn is not None
        cur = await self.conn.execute(
            """SELECT l.*, c.name AS category_name, u.full_name AS seller_name, u.username AS seller_username
               FROM listings l JOIN categories c ON c.id = l.category_id JOIN users u ON u.user_id = l.user_id
               WHERE l.status = 'approved' ORDER BY l.created_at DESC LIMIT ? OFFSET ?;""",
            (limit, offset),
        )
...
def main_menu_kb(is_admin: bool = False):
    ...
    b.button(text="🔵 آخرین آگهی‌ها", callback_data="menu:latest:0")
    if is_admin:
...
def latest_listings_nav_kb(offset: int, has_more: bool):
    b = InlineKeyboardBuilder()
    if offset > 0:
        prev_offset = max(0, offset - LATEST_LISTINGS_LIMIT)
        b.button(text="◀️ قبلی", callback_data=f"menu:latest:{prev_offset}")
    if has_more:
        b.button(text="▶️ بعدی", callback_data=f"menu:latest:{offset + LATEST_LISTINGS_LIMIT}")
...
def categories_kb(categories):
    ...
    for cat in categories:
        b.button(text=f"🔵 {cat['name']}", callback_data=f"cat:{cat['id']}")
...
def admin_panel_kb():
    ...
    b.button(text="🟢 رفع مسدودی کاربر", callback_data="admin:unban")
    b.button(text="🔴 افزودن کلمه فیلترشده", callback_data="admin:addword")
    b.button(text="🔵 مدیریت دسته‌بندی‌ها", callback_data="admin:categories")
    b.button(text="🔴 بازگشت به منو", callback_data="menu:back")
    b.adjust(2, 2, 2, 1, 1)
    return b.as_markup()


def category_management_kb(categories):
    b = InlineKeyboardBuilder()
    for cat in categories:
        b.button(text=f"🔴 حذف: {cat['name']}", callback_data=f"admin:catdel:{cat['id']}")
    b.button(text="🟢 افزودن دسته‌بندی جدید", callback_data="admin:catadd")
    b.button(text="🔴 بازگشت به پنل", callback_data="menu:admin")
    b.adjust(1)
    return b.as_markup()
...
@browse_router.callback_query(F.data.startswith("menu:latest:"))
async def latest_listings(callback: CallbackQuery) -> None:
    ...
    listings = await db.get_latest_listings(limit=LATEST_LISTINGS_LIMIT + 1, offset=offset)
    has_more = len(listings) > LATEST_LISTINGS_LIMIT
    listings = listings[:LATEST_LISTINGS_LIMIT]

    if not listings and offset == 0:
        await callback.answer("فعلاً هیچ آگهی تأییدشده‌ای وجود ندارد.", show_alert=True)
        return
    if not listings:
        await callback.answer("آگهی بیشتری وجود ندارد.", show_alert=True)
        return
    ...
    if callback.message:
        ...
        await callback.message.answer(
            f"📄 صفحه {offset // LATEST_LISTINGS_LIMIT + 1}",
            reply_markup=latest_listings_nav_kb(offset, has_more),
        )
...
async def open_category_management(callback: CallbackQuery) -> None:
    categories = await db.get_categories()
    if callback.message:
        await callback.message.edit_text(
            "📂 مدیریت دسته‌بندی‌ها:\n\nروی هر دسته بزنید تا حذف شود "
            "(اگه آگهی فعالی توش باشه، اجازه حذف داده نمی‌شه).",
            reply_markup=category_management_kb(categories),
        )
    await callback.answer()


@admin_router.callback_query(F.data == "admin:catadd")
async def start_add_category(callback: CallbackQuery, state: FSMContext) -> None:
    if callback.message:
        await callback.message.answer("🟢 اسم دسته‌بندی جدید را وارد کنید:", reply_markup=cancel_kb())
    await state.set_state(AdminForm.adding_category)
    await callback.answer()


@admin_router.message(AdminForm.adding_category, F.text)
async def do_add_category(message: Message, state: FSMContext) -> None:
    await state.clear()
    name = message.text.strip()
    if not (2 <= len(name) <= 50):
        await message.answer("⚠️ اسم دسته‌بندی باید بین ۲ تا ۵۰ کاراکتر باشد.")
        return
    success = await db.add_category(name)
    if success:
        await message.answer(f"✅ دسته‌بندی «{name}» اضافه شد.", reply_markup=admin_panel_kb())
    else:
        await message.answer(f"⚠️ دسته‌بندی «{name}» از قبل وجود دارد.", reply_markup=admin_panel_kb())


@admin_router.callback_query(F.data.startswith("admin:catdel:"))
async def do_delete_category(callback: CallbackQuery) -> None:
    category_id = int(callback.data.split(":")[2])
    success = await db.delete_category(category_id)
    if success:
        await callback.answer("🗑 دسته‌بندی حذف شد.", show_alert=True)
    else:
        await callback.answer(
            "⚠️ این دسته‌بندی حذف نشد چون آگهی فعالی داخلش وجود دارد.", show_alert=True
        )
    categories = await db.get_categories()
    if callback.message:
        await callback.message.edit_text(
            "📂 مدیریت دسته‌بندی‌ها:", reply_markup=category_management_kb(categories)
        )
